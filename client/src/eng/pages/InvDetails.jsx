...
const InvDetails = () => {
    ...
    // Update contact balance function
    const updateContactBalance = async (contact, amount, currency, isCustomer = true) => {
        try {
            const response = await axios.patch(`/v1/api/contacts/${contact._id}/balance`, {
                balance: amount + (contact.balance || 0),
                balanceCurrency: currency
            });

            if (response.data.success) {
                console.log(`${isCustomer ? 'Customer' : 'Supplier'} balance updated successfully`);
                return true;
            } else {
                console.error('Failed to update balance:', response.data);
                return false;
            }
        } catch (error) {
        ...
        try {
            ...
            if (response.data.success) {
                ...
                if (newStatus.toLowerCase() === 'order') {
                    toast.success('Create a new order successfully');
                    ...
                    <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
                        ...
                        <div className="relative">
                            ...
                            >
                                ...
                                <span>{invoice.status || invoice.invoiceStatus || 'N/A'}</span>
                                ...
                                <div className="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-xl border border-gray-200 py-1 z-20">
                                    {statusOptions.map((status) => (
                                        <button
                                            ...
                                            disabled={updating}
                                            className="w-full text-left px-4 py-2 text-sm hover:bg-blue-50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                                        >
                                            {status}
                                        </button>
                                    ))}
                    ...
                    <div className="lg:col-span-2 space-y-6">
                        ...
                        <div className="bg-white rounded-xl shadow-lg p-6 border border-gray-200">
                            <h3 className="text-lg font-semibold text-gray-800 mb-4 flex items-center gap-2">
                                ...
                                {invoice.customer ? 'Customer Information' : 'Supplier Information'}
                            </h3>
                                            ...
                                            invoice.items.map((item, index) => (
                                                <tr key={index} className="hover:bg-gray-50">
                                                    ...
                                                    <td className="px-4 py-3 text-right text-sm font-medium text-gray-900">
                                                        {(item.price * item.quantity)?.toFixed(2)} {invoice.bills?.currency}
                                                    </td>
                                                </tr>
                                            ))
                                        ) : (
