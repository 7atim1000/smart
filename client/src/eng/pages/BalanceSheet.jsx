...
const BalanceSheet = ({ isRTL = false }) => {
    ...
    const fetchBalanceSheet = async () => {
        ...
        try {
            ...
            if (response.data.success) {
                ...
                toast.error(isRTL ? 'فشل تحميل الميزانية العمومية' : 'Failed to load balance sheet');
            }
        } catch (error) {
            console.error('Error fetching balance sheet:', error);
            toast.error(isRTL ? 'خطأ في تحميل الميزانية العمومية' : 'Error loading balance sheet');
        } finally {
            setLoading(false);
        }
    };
    ...
    const renderAssets = () => {
        ...
        const { total, byLevel, accounts } = balanceSheetData.assets;
                            ...
                            >
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="font-semibold">
                                        {isRTL ? byLevel[level].levelNameArb : byLevel[level].levelName}
                                    </span>
                                </div>
                                        ...
                                        <div key={className} className="mb-3">
                                            ...
                                            <div className="flex items-center justify-between bg-gray-50 p-2 rounded">
                                                <span className="font-medium text-gray-700">
                                                    {isRTL ? byLevel[level].classes[className].classNameArb : className}
                                                </span>
                                                    ...
                                                    <div key={groupName} className="mb-2">
                                                        ...
                                                        <div className="flex items-center justify-between text-sm">
                                                            <span className="text-gray-600">
                                                                {isRTL ? byLevel[level].classes[className].groups[groupName].groupNameArb : groupName}
                                                            </span>
                                                            <span className="font-medium">
                                                                {formatNumber(
                                                        ...
                                                        <div className="mr-4 mt-1">
                                                            {byLevel[level].classes[className].groups[groupName].accounts.map((account, idx) => (
                                                                <div key={idx} className="flex items-center justify-between text-xs text-gray-500 py-1">
                                                                    <span>
                                                                        {isRTL ? account.nameArb : account.name}
                                                                    </span>
        ...
        return (
            <div className="border rounded-lg overflow-hidden">
                <div className="bg-orange-600 text-white px-4 py-3 font-bold flex justify-between items-center">
                    <span>{isRTL ? 'الخصوم' : 'Liabilities'}</span>
                    <span className="text-lg">{formatNumber(total)}</span>
                </div>
                <div className="p-4">
                    {Object.keys(byLevel || {}).map(level => (
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                <div>
                    ...
                    <p className="text-sm sm:text-base text-gray-600 mt-1">
                        {isRTL ? 'عرض الميزانية العمومية للحسابات' : 'View balance sheet for accounts'}
                    </p>
                </div>
                            ...
                            {Math.abs(calculateDifference()) > 0.01 && (
                                <p className="text-sm text-red-500 mt-2 text-center">
                                    {isRTL 
                                        ...
                                        : 'Warning: Balance sheet is not balanced! Total assets should equal total liabilities + equity.'}
                                </p>
                            )}
                        </div>
                    )}
