                ...
                <div className="p-6 overflow-y-auto max-h-[calc(90vh-80px)]">
                    {loading ? (
                        ...
                        <div className="text-center py-12">
                            ...
                            <p className="text-gray-500">No journal entries found for "{journalName}"</p>
                        </div>
                    ) : (
                        <>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div>
                                            <div className="flex items-center gap-2 mb-2">
                                                ...
                                                <span className="font-semibold text-gray-700">Journal Details:</span>
                                            </div>
                                            <p className="text-sm text-gray-600 ml-6">
                                                <span className="font-medium">Code:</span> {selectedJournal.code || 'N/A'}
                                            </p>
                                            <p className="text-sm text-gray-600 ml-6">
                                                <span className="font-medium">Reference:</span> {selectedJournal.reference || 'N/A'}
                                            </p>
                                            <p className="text-sm text-gray-600 ml-6">
                                                <span className="font-medium">Period:</span> {selectedJournal.period || 'N/A'}
                                            </p>
                                            <p className="text-sm text-gray-600 ml-6">
                                                <span className="font-medium">Fiscal Year:</span> {selectedJournal.fiscalYear || 'N/A'}
                                            </p>
                                        </div>
                                        <div>
                                            <div className="flex items-center gap-2 mb-2">
                                            ...
                                            <p className="text-sm text-gray-600 ml-6">
                                                <span className="font-medium">Created:</span> {formatDate(selectedJournal.createdAt)}
                                            </p>
                                            <p className="text-sm text-gray-600 ml-6">
                                                <span className="font-medium">Last Entry:</span> {formatDate(selectedJournal.lastEntryDate)}
                                            </p>
                                            <p className="text-sm text-gray-600 ml-6">
                            ...
                            {entries.length > 0 ? (
                                    ...
                                    <table className="w-full border-collapse">
                                        <thead>
                                            <tr className="bg-gray-100">
                                                ...
                                                <th className="border p-3 text-left text-sm font-semibold text-gray-700">Reference</th>
                                                <th className="border p-3 text-left text-sm font-semibold text-gray-700">Description</th>
                                                <th className="border p-3 text-left text-sm font-semibold text-gray-700">Account Name</th>
                                                <th className="border p-3 text-right text-sm font-semibold text-gray-700">Debit</th>
                                                <th className="border p-3 text-right text-sm font-semibold text-gray-700">Credit</th>
                                                <th className="border p-3 text-right text-sm font-semibold text-gray-700">Balance</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                                ...
                                                <tr key={index} className="hover:bg-gray-50 transition-colors">
                                                    ...
                                                    <td className="border p-3 text-sm text-gray-700">
                                                        <div>
                                                            ...
                                                            {entry.descriptionArb && (
                                                                <p className="text-xs text-gray-500">{entry.descriptionArb}</p>
                                                            )}
                                                        </div>
                                                     </td>
                                                    <td className="border p-3 text-sm text-gray-700">
                                                        <div>
                                                            <p className="font-medium">{entry.accName}</p>
                                                            {entry.accNameArb && (
                                                                <p className="text-xs text-gray-500">{entry.accNameArb}</p>
                                                            )}
                                                        </div>
                                                        {entry.partnerName && (
                                                    ...
                                                    <td className="border p-3 text-right text-sm font-medium text-green-600">
                                                        {entry.debit > 0 ? formatCurrency(entry.debit) : '-'}
                                                     </td>
                                                    <td className="border p-3 text-right text-sm font-medium text-red-600">
                                                        {entry.credit > 0 ? formatCurrency(entry.credit) : '-'}
                                                     </td>
                                                    <td className="border p-3 text-right text-sm font-medium text-gray-700">
                                        ...
                                        <tfoot className="bg-gray-100 font-semibold">
                                            <tr>
                                                <td colSpan="4" className="border p-3 text-right font-bold text-gray-800">
                                                    Totals:
                                                </td>
                                                <td className="border p-3 text-right text-green-700 font-bold">
                                                    {formatCurrency(summary.totalDebit)}
                                                </td>
                                                <td className="border p-3 text-right text-red-700 font-bold">
                                                    {formatCurrency(summary.totalCredit)}
                                                </td>
                                                <td className="border p-3 text-right font-bold text-gray-800">
                                ...
                                <div className="text-center py-12">
                                    ...
                                    <p className="text-gray-500">No entries found for this journal</p>
                                </div>
                            )}
