...
const Bills = ({ fetchServices }) => {
    ...
    const handlePlaceOrder = async () => {
        ...
        // Validate items
        if (buyData.length === 0) {
            toast.error('الرجاء اختيار العناصر');
            setIsProcessing(false);
            return;
        }
        ...
        try {
            ...
            if (response.data.success) {
                toast.success('تم إنشاء عرض سعر الشراء بنجاح!');
                ...
                toast.error('فشل في إنشاء عرض السعر');
            }
        } catch (error) {
            console.error('Error creating quotation:', error);
            toast.error(error.response?.data?.message || 'خطأ في إنشاء عرض سعر الشراء');
        } finally {
            setIsProcessing(false);
        }
    };
    ...
    const cancelOrder = () => {
        if (buyData.length === 0) {
            toast('لا توجد عناصر للإلغاء', {
                icon: 'ℹ️'
            });
            return;
        }

        if (window.confirm('هل أنت متأكد من إلغاء جميع العناصر؟')) {
            dispatch(removeSupplier());
            dispatch(removeAllItems());
            if (fetchServices) {
                fetchServices({ category: 'all', page: 1 });
    ...
    return (
        <div className='bg-white rounded-xl shadow-lg border border-blue-100 p-4 md:p-1 h-full' dir="rtl">
            {/* Summary Cards */}
            <div className='grid grid-cols-2 gap-3 mb-4'>
                <div className='bg-blue-50 rounded-lg p-3 border border-blue-100'>
                    <p className='text-xs text-blue-600 mb-1'>العناصر</p>
                    <p className='text-xl font-bold text-blue-800'>{buyData.length}</p>
                    <p className='text-xs text-gray-500'>منتج</p>
                </div>

                <div className='bg-blue-50 rounded-lg p-3 border border-blue-100'>
                    <p className='text-xs text-blue-600 mb-1'>المجموع الفرعي</p>
            ...
            <div className='space-y-2 mb-4'>
                ...
                <div className='flex justify-between items-center py-3 bg-blue-50 rounded-lg px-3'>
                    <span className='text-sm font-semibold text-blue-800'>الإجمالي النهائي:</span>
            ...
            <div className='mb-4 p-3 bg-yellow-50 rounded-lg border border-yellow-200'>
                <div className='flex items-center gap-2 mb-2'>
                    ...
                    <span className='text-sm font-medium text-gray-700'>الرصيد المستحق</span>
                </div>
                <div className='flex justify-between items-center'>
                    <span className='text-sm text-gray-600'>المبلغ المراد دفعه:</span>
                    <span className='text-lg font-bold text-red-600'>
                        {balance.toFixed(2)} {currency}
                    </span>
                </div>
                <p className='text-xs text-gray-500 mt-2'>
                    هذا عرض سعر. سيتم تحصيل الدفع بشكل منفصل.
                </p>
            </div>
            ...
            <div className='mt-4 pt-3 border-t border-blue-100'>
                <div className='flex items-center gap-2'>
                    ...
                    <span className='text-xs text-gray-600'>
                        {buyData.length > 0
                            ? `جاهز لإنشاء عرض السعر - ${buyData.length} عناصر`
                            : 'لا توجد عناصر لعرض السعر'}
                    </span>
                </div>
            </div>
