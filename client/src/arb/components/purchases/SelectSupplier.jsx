...
const SelectSupplier = ({ setIsSelectSupplierModalOpen }) => {
    ...
    const handleClose = (supplierId, supplierName, email, balance, currency) => {
    ...
    // Fetch suppliers function with useCallback to prevent unnecessary recreations
    const fetchContacts = useCallback(async (page = pagination.currentPage) => {
        ...
        try {
            const params = new URLSearchParams({
                ...
                // Only fetch suppliers (isPayable = true)
                isPayable: 'true'
            });
    ...
    // Fetch suppliers when debounced search or sort changes
    useEffect(() => {
    ...
    return (
        <div className='fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-1' dir="rtl">
            <motion.div
            ...
            >
                ...
                <div className="flex items-center justify-between p-2 bg-gradient-to-r from-blue-600 to-blue-700 rounded-t-2xl">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className='text-white text-lg md:text-xl font-bold'>اختيار المورد</h2>
                            <p className='text-blue-100 text-sm'>اختر مورداً للمشتريات الخاصة بك</p>
                        </div>
                    </div>
                    <button
                ...
                <div className='p-6 border-b border-blue-100'>
                    <div className='flex flex-col md:flex-row gap-4'>
                        <div className="relative flex-1">
                            ...
                            <input
                                type='text'
                                placeholder='البحث عن موردين بالاسم أو البريد الإلكتروني أو رقم الهاتف...'
                                className='w-full pr-10 pl-4 py-3 border border-blue-200 rounded-xl bg-white text-gray-700 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 focus:outline-none transition duration-200'
                                value={search}
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            >
                                ...
                                <span className="text-sm font-medium">
                                    ترتيب: {sort === '-createdAt' ? 'الأحدث' : 'الأقدم'}
                                </span>
                            </button>
                ...
                {loading && (
                    <div className="flex flex-col items-center justify-center py-12">
                        ...
                        <p className="text-gray-600 font-medium">جاري تحميل الموردين...</p>
                        <p className="text-sm text-gray-500 mt-2">يرجى الانتظار أثناء جلب قائمة الموردين</p>
                    </div>
                )}
                    ...
                    {!loading && list.length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-16 px-4 text-center">
                            ...
                            <h3 className="text-xl font-semibold text-gray-700 mb-2">
                                {debouncedSearch ? 'لا توجد موردين' : 'لا يوجد موردين متاحين'}
                            </h3>
                            <p className="text-gray-500 max-w-md">
                                {debouncedSearch
                                    ? `لم يتم العثور على موردين يطابقون "${debouncedSearch}"`
                                    : 'قائمة الموردين فارغة. ابدأ بإضافة موردين جدد!'}
                            </p>
                        </div>
                    ) : (
                                    ...
                                    >
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    ...
                                                    <div className="flex items-center gap-2 mt-1">
                                                        <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                                                            ...
                                                            {Number(supplier.balance) === 0 ? 'رصيد صفري' : 
                                                             Number(supplier.balance) > 0 ? 'يوجد رصيد' : 'رصيد دائن'}
                                                        </span>
                                                    </div>
                                                </div>
                                            </div>
                                        ...
                                        <div className="space-y-3">
                                            ...
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    <p className="text-xs text-gray-500">رقم الاتصال</p>
                ...
                <div className='p-4 border-t border-blue-100 bg-gradient-to-r from-blue-50 to-white rounded-b-2xl'>
                    <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
                        <div className="flex items-center gap-2 text-sm text-gray-600">
                            ...
                            <span>عرض <span className="font-semibold text-blue-700">{list.length}</span> مورد</span>
                        </div>
                        <div className="flex items-center gap-4">
                            ...
                            <div className="text-xs text-gray-500">
                                انقر على بطاقة المورد للاختيار
                            </div>
                        </div>
                    </div>
                </div>
