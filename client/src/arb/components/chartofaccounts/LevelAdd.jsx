...
import { FaPlus, FaLayerGroup, FaEdit } from "react-icons/fa";
...
const LevelAdd = ({ 
    ...
    const validateForm = () => {
        ...
        if (!name.trim()) {
            errors.name = 'اسم المستوى بالإنجليزية مطلوب';
        }
        
        if (!nameArb.trim()) {
            errors.nameArb = 'اسم المستوى بالعربية مطلوب';
        }
    ...
    const onSubmitHandler = async (e) => {
        ...
        try {
            ...
            if (response.data.success) {
                toast.success(mode === 'edit' ? 'تم تحديث المستوى بنجاح' : 'تم إضافة المستوى بنجاح');
                setName('');
                setNameArb('');
                setCode('');
                setFormErrors({});
            ...
            toast.error(error.response?.data?.message || (mode === 'edit' ? 'فشل في تحديث المستوى' : 'فشل في إضافة المستوى'));
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const getModalTitle = () => {
        if (mode === 'edit') return 'تعديل المستوى';
        return 'إضافة مستوى جديد';
    };

    const getModalDescription = () => {
        if (mode === 'edit') return 'تحديث معلومات المستوى';
        return 'إنشاء مستوى جديد في هذا المستوى الرئيسي';
    };

    const getButtonText = () => {
        if (isLoading) return mode === 'edit' ? 'جاري التحديث...' : 'جاري الإنشاء...';
        return mode === 'edit' ? 'تحديث المستوى' : 'إضافة المستوى';
    };
                ...
                >
                    ...
                    <form onSubmit={onSubmitHandler} className="p-6">
                        ...
                        <div className="mb-5 p-3 bg-blue-50 rounded-lg border border-blue-200">
                            <p className="text-xs text-blue-600 mb-1">المستوى الرئيسي:</p>
                            <p className="text-sm font-semibold text-blue-800">
                                {chartId ? 'جاري الإنشاء في المستوى الرئيسي' : 'معلومات التسلسل الهرمي'}
                            </p>
                        </div>

                        {/* Level Arabic Name (Primary) */}
                        <div className="mb-5">
                            <label className="block text-sm font-semibold text-gray-700 mb-2 text-right">
                                اسم المستوى بالعربية <span className="text-red-500">*</span>
                            </label>
                            <input
                                ref={inputRef}
                                type="text"
                        ...
                        <div className="mb-5">
                            <label className="block text-sm font-semibold text-gray-700 mb-2 text-right">
                                اسم المستوى بالإنجليزية <span className="text-red-500">*</span>
                            </label>
                            <input
                                type="text"
                                value={name}
                        ...
                        <div className="mb-6">
                            ...
                            {!formErrors.code && (
                                <p className="mt-1 text-xs text-gray-500 text-right">
                                    يجب أن يكون الكود فريداً داخل هذا المستوى الرئيسي
                                </p>
                            )}
                        </div>
                        ...
                        <div className="mb-6 p-4 bg-gradient-to-r from-blue-50 to-blue-100 rounded-lg border border-blue-300 shadow-inner">
                            ...
                            <p className="text-xs text-blue-700 text-right">
                                المستويات هي المستوى الأول في التسلسل الهرمي (بعد المستوى الرئيسي). 
                                يمكن إضافة التصنيفات تحت هذه المستويات.
                            </p>
                        </div>
                            ...
                            >
                                {isLoading ? (
                                    ...
                                    <>
                                        ...
                                        <span>{mode === 'edit' ? 'تحديث المستوى' : 'إضافة المستوى'}</span>
                                    </>
                                )}
                            </button>
                        </div>
