...
const Bills = ({ fetchServices }) => {
    ...
    const currency = saleData[0]?.currency || 'AED';
    ...
    const handlePlaceOrder = async () => {
        ...
        if (saleData.length === 0) {
            toast.error('الرجاء اختيار منتجات');
            setIsProcessing(false);
            return;
        }
        ...
        try {
            ...
            toast.error(error.response?.data?.message || 'خطأ في إنشاء عرض السعر');
        } finally {
            setIsProcessing(false);
        }
    };
        ...
        <div className='bg-white rounded-xl shadow-lg border border-blue-100 p-4 md:p-1 h-full' dir="rtl">
            ...
            <div className='grid grid-cols-2 gap-3 mb-4'>
                <div className='bg-blue-50 rounded-lg p-3 border border-blue-100'>
                    <p className='text-xs text-blue-600 mb-1'>المنتجات</p>
                    <p className='text-xl font-bold text-blue-800'>{saleData.length}</p>
                    <p className='text-xs text-gray-500'>عنصر</p>
                </div>
            ...
            <div className='mb-4 p-3 bg-yellow-50 rounded-lg border border-yellow-200'>
                ...
                <div className='flex justify-between items-center'>
                    <span className='text-sm text-gray-600'>المطلوب دفعه:</span>
            ...
            <div className='mt-4 pt-3 border-t border-blue-100'>
                <div className='flex items-center gap-2'>
                    ...
                    <span className='text-xs text-gray-600'>
                        {saleData.length > 0
                            ? `جاهز لإنشاء عرض السعر - ${saleData.length} عنصر`
                            : 'لا توجد عناصر لإنشاء عرض سعر'}
                    </span>
                </div>
            </div>

            {/* Quotation Modal */}
            {showQuotation && saleInfo && (
                <PrintQuotation
                    saleInfo={saleInfo}
