...
const arabicText = {
    // Page Header
    journalsManagement: 'إدارة القيود',
    manageJournals: 'إدارة وتتبع وإضافة القيود المحاسبية',
    ...
    // Empty State
    noJournalsFound: 'لم يتم العثور على قيود',
    noJournalsMatch: 'لا توجد قيود تطابق معايير البحث',
    getStarted: 'ابدأ بإضافة أول قيد',
    
    // Table Headers
    codeName: 'الرمز / الاسم',
    status: 'الحالة',
    period: 'الفترة',
    currency: 'العملة',
    debitCredit: 'مدين / دائن',
    balance: 'الرصيد',
    entries: 'الإدخالات',
    actions: 'الإجراءات',
    ...
    // Toast Messages
    deleteSuccess: 'تم حذف القيد بنجاح',
    deleteFailed: 'فشل حذف القيد',
    deleteError: 'خطأ في حذف القيد',
    fetchFailed: 'فشل في جلب القيود',
    fetchError: 'خطأ في تحميل القيود',
...
const statusOptions = [
    { value: 'all', label: 'All Statuses', labelArb: arabicText.allStatuses },
    { value: 'draft', label: 'Draft', labelArb: arabicText.draft },
    { value: 'posted', label: 'Posted', labelArb: arabicText.posted },
];

const Journals = ({ isRTL = true }) => {
    ...
    const fetchJournals = async () => {
        ...
        try {
            ...
            if (response.data.success) {
                ...
                toast.error(isRTL ? arabicText.fetchFailed : 'Failed to fetch journals');
            }
        } catch (error) {
            console.error('Error fetching journals:', error);
            toast.error(isRTL ? arabicText.fetchError : 'Error loading journals');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleDeleteJournal = async (id, e) => {
        e.stopPropagation();
        if (window.confirm(isRTL ? arabicText.confirmDelete : 'Are you sure you want to delete this journal?')) {
            try {
                const response = await axios.delete(`/v1/api/journals/${id}`);
                if (response.data.success) {
                    toast.success(isRTL ? arabicText.deleteSuccess : 'Journal deleted successfully');
                    fetchJournals();
                } else {
                    toast.error(isRTL ? arabicText.deleteFailed : 'Failed to delete journal');
                }
            } catch (error) {
                console.error('Delete error:', error);
                toast.error(isRTL ? arabicText.deleteError : 'Error deleting journal');
            }
        }
    };
    ...
    const getStatusBadge = (journal) => {
        if (journal.isClosed) {
            return <span className="px-2 py-1 bg-gray-100 text-gray-600 text-xs rounded-full">{isRTL ? arabicText.closed : 'Closed'}</span>;
        }
        if (!journal.isActive) {
            return <span className="px-2 py-1 bg-red-100 text-red-600 text-xs rounded-full">{isRTL ? arabicText.inactive : 'Inactive'}</span>;
        }
        ...
        if (journal.status) {
            ...
            const statusLabels = {
                draft: isRTL ? arabicText.draft : 'draft',
                posted: isRTL ? arabicText.posted : 'posted',
                approved: isRTL ? arabicText.approved : 'approved',
                rejected: isRTL ? arabicText.rejected : 'rejected',
                cancelled: isRTL ? arabicText.cancelled : 'cancelled'
            };
            
            const colorClass = statusColors[journal.status] || 'bg-green-100 text-green-600';
            return <span className={`px-2 py-1 ${colorClass} text-xs rounded-full capitalize`}>{statusLabels[journal.status] || journal.status}</span>;
        }
        
        return <span className="px-2 py-1 bg-green-100 text-green-600 text-xs rounded-full">{isRTL ? arabicText.active : 'Active'}</span>;
    };
        ...
        <div dir={isRTL ? 'rtl' : 'ltr'} className="p-4 sm:p-6 max-w-7xl mx-auto w-full">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                <div>
                    <h1 className="text-2xl sm:text-3xl font-bold text-gray-800 flex items-center gap-2">
                        ...
                        {isRTL ? arabicText.journalsManagement : 'Journals Management'}
                    </h1>
                    <p className="text-sm sm:text-base text-gray-600 mt-1">
                        {isRTL ? arabicText.manageJournals : 'Manage, add and track accounting journals'}
                    </p>
                </div>
                ...
                <div className="flex gap-3">
                    ...
                    <div className="flex bg-gray-100 rounded-lg p-1">
                        <button
                            ...
                            title={isRTL ? arabicText.gridView : 'Grid view'}
                        >
                        ...
                        <button
                            ...
                            title={isRTL ? arabicText.listView : 'List view'}
                        >
                            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    >
                        <FaPlus size={16} />
                        {isRTL ? arabicText.addJournal : 'Add Journal'}
                    </button>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
                <div className="bg-gradient-to-br from-blue-50 to-blue-100 rounded-xl p-4 border border-blue-200">
                    <p className="text-sm text-blue-600 mb-1">{isRTL ? arabicText.totalJournals : 'Total Journals'}</p>
                    <p className="text-2xl font-bold text-blue-800">{journals.length}</p>
                </div>
                <div className="bg-gradient-to-br from-green-50 to-green-100 rounded-xl p-4 border border-green-200">
                    <p className="text-sm text-green-600 mb-1">{isRTL ? arabicText.activeJournals : 'Active Journals'}</p>
                    <p className="text-2xl font-bold text-green-800">
                        {journals.filter(j => j.isActive && !j.isClosed).length}
                    </p>
                </div>
                <div className="bg-gradient-to-br from-purple-50 to-purple-100 rounded-xl p-4 border border-purple-200">
                    <p className="text-sm text-purple-600 mb-1">{isRTL ? arabicText.totalBalance : 'Total Balance'}</p>
                    <p className="text-2xl font-bold text-purple-800">
                        {journals.reduce((sum, j) => sum + (j.currentBalance || 0), 0).toFixed(2)}
                    </p>
                </div>
                <div className="bg-gradient-to-br from-amber-50 to-amber-100 rounded-xl p-4 border border-amber-200">
                    <p className="text-sm text-amber-600 mb-1">{isRTL ? arabicText.totalEntries : 'Total Entries'}</p>
                    <p className="text-2xl font-bold text-amber-800">
                        {journals.reduce((sum, j) => sum + (j.entries?.length || 0), 0)}
                    </p>
                </div>
                <div className="bg-gradient-to-br from-indigo-50 to-indigo-100 rounded-xl p-4 border border-indigo-200">
                    <p className="text-sm text-indigo-600 mb-1">{isRTL ? arabicText.statusTypes : 'Status Types'}</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                ...
                <div className="relative md:col-span-1">
                    <input
                        ...
                        placeholder={isRTL ? arabicText.searchPlaceholder : 'Search journals...'}
                        className={`w-full ${isRTL ? 'pr-10 pl-4' : 'pl-10 pr-4'} py-3 border-2 border-gray-200 rounded-lg focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-200 transition-all`}
                    />
                    <FaSearch className={`absolute ${isRTL ? 'right-3' : 'left-3'} top-1/2 transform -translate-y-1/2 text-gray-400`} size={18} />
                </div>
                ...
                <div className="relative">
                    <select
                        ...
                        className="w-full px-4 py-3 border-2 border-gray-200 rounded-lg focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-200 appearance-none bg-white"
                    >
                        <option value="all">{isRTL ? arabicText.allPeriods : 'All Periods'}</option>
                    ...
                    <FaCalendarAlt className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400" size={18} />
                </div>
                ...
                <div className="relative">
                    <select
                        ...
                        className="w-full px-4 py-3 border-2 border-gray-200 rounded-lg focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-200 appearance-none bg-white"
                    >
                        <option value="all">{isRTL ? arabicText.allYears : 'All Years'}</option>
            ...
            <div className="flex flex-wrap items-center justify-between gap-4 mb-4">
                <div className="flex items-center gap-2">
                    <span className="text-sm text-gray-600">
                        {isRTL ? arabicText.showing : 'Showing'} {filteredJournals.length} {isRTL ? arabicText.of : 'of'} {journals.length} {isRTL ? arabicText.journals : 'journals'}
                    </span>
                    ...
                    {(statusFilter !== 'all' || periodFilter !== 'all' || yearFilter !== 'all' || searchTerm) && (
                        ...
                        >
                            <MdRefresh size={14} />
                            {isRTL ? arabicText.clearFilters : 'Clear filters'}
                        </button>
                    )}
                </div>
                ...
                <div className="flex items-center gap-2 text-sm text-gray-500">
                    ...
                    <span>{isRTL ? arabicText.currencies : 'Currencies'}: {availableCurrencies.map(c => getCurrencyFlag(c)).join(' ')}</span>
                </div>
            </div>
            ...
            {loading ? (
                <div className="text-center py-12">
                    ...
                    <p className="mt-4 text-gray-600">{isRTL ? arabicText.loading : 'Loading journals...'}</p>
                </div>
            ) : filteredJournals.length === 0 ? (
                <div className="text-center py-12 bg-gray-50 rounded-xl">
                    <FaFileInvoice className="mx-auto text-5xl text-gray-400 mb-4" />
                    <h3 className="text-xl font-semibold text-gray-700 mb-2">{isRTL ? arabicText.noJournalsFound : 'No Journals Found'}</h3>
                    <p className="text-gray-500 mb-4">
                        {searchTerm || statusFilter !== 'all' || periodFilter !== 'all' || yearFilter !== 'all'
                            ? (isRTL ? arabicText.noJournalsMatch : 'No journals match your filters')
                            : (isRTL ? arabicText.getStarted : 'Get started by adding your first journal')}
                    </p>
                    {!searchTerm && statusFilter === 'all' && periodFilter === 'all' && yearFilter === 'all' && (
                        <button
                            onClick={() => setIsAddModalOpen(true)}
                        ...
                        >
                                ...
                                <div className="flex items-center justify-between">
                                    ...
                                    <div className="flex gap-2" onClick={(e) => e.stopPropagation()}>
                                        <button
                                            ...
                                            title={isRTL ? arabicText.edit : 'Edit'}
                                        >
                                            <FaEdit size={14} />
                                        </button>
                                        <button
                                            ...
                                            className="p-1.5 bg-white/20 rounded-lg hover:bg-white/30 transition-colors text-white"
                                            title={isRTL ? arabicText.delete : 'Delete'}
                                        >
                                            <FaTrash size={14} />
                                        </button>
                                    </div>
                            ...
                            <div className="p-4">
                                ...
                                <div className="flex items-center justify-between text-sm bg-blue-50 p-2 rounded-lg mb-3">
                                    <span className="text-gray-600">{isRTL ? arabicText.periodLabel : 'Period:'}</span>
                                    <span className="font-medium text-blue-800">{journal.period}</span>
                                    <span className="text-gray-600">{isRTL ? arabicText.yearLabel : 'Year:'}</span>
                                ...
                                <div className="grid grid-cols-3 gap-2 mb-3">
                                    <div className="text-center p-2 bg-blue-50 rounded-lg">
                                        <p className="text-xs text-blue-600">{isRTL ? arabicText.debit : 'Debit'}</p>
                                        <p className="font-bold text-blue-800">{formatCurrency(journal.totalDebit, journal.currency)}</p>
                                    </div>
                                    <div className="text-center p-2 bg-orange-50 rounded-lg">
                                        <p className="text-xs text-orange-600">{isRTL ? arabicText.credit : 'Credit'}</p>
                                ...
                                <div className="flex items-center justify-between p-3 rounded-lg bg-gray-50 mb-3">
                                    <span className="font-medium text-gray-700">{isRTL ? arabicText.currentBalance : 'Current Balance'}</span>
                                ...
                                <div className="mt-2 text-center text-xs text-blue-600 hover:text-blue-800">
                                    {isRTL ? arabicText.clickToView : 'Click to view full details'}
                                </div>
                            </div>
                        </motion.div>
                    ))}
                        ...
                        <thead className="bg-gray-50">
                            <tr>
                                <th className={`px-6 py-3 text-${isRTL ? 'right' : 'left'} text-xs font-medium text-gray-500 uppercase tracking-wider`}>
                                    {isRTL ? arabicText.codeName : 'Code / Name'}
                                </th>
                                <th className={`px-6 py-3 text-${isRTL ? 'right' : 'left'} text-xs font-medium text-gray-500 uppercase tracking-wider`}>
                                    {isRTL ? arabicText.status : 'Status'}
                                </th>
                                <th className={`px-6 py-3 text-${isRTL ? 'right' : 'left'} text-xs font-medium text-gray-500 uppercase tracking-wider`}>
                                    {isRTL ? arabicText.period : 'Period'}
                                </th>
                                <th className={`px-6 py-3 text-${isRTL ? 'right' : 'left'} text-xs font-medium text-gray-500 uppercase tracking-wider`}>
                                    {isRTL ? arabicText.currency : 'Currency'}
                                </th>
                                <th className={`px-6 py-3 text-${isRTL ? 'right' : 'left'} text-xs font-medium text-gray-500 uppercase tracking-wider`}>
                                    {isRTL ? arabicText.debitCredit : 'Debit / Credit'}
                                </th>
                                <th className={`px-6 py-3 text-${isRTL ? 'right' : 'left'} text-xs font-medium text-gray-500 uppercase tracking-wider`}>
                                    {isRTL ? arabicText.balance : 'Balance'}
                                </th>
                                <th className={`px-6 py-3 text-${isRTL ? 'right' : 'left'} text-xs font-medium text-gray-500 uppercase tracking-wider`}>
                                    {isRTL ? arabicText.entries : 'Entries'}
                                </th>
                                <th className={`px-6 py-3 text-${isRTL ? 'right' : 'left'} text-xs font-medium text-gray-500 uppercase tracking-wider`}>
                                    {isRTL ? arabicText.actions : 'Actions'}
                                </th>
                            </tr>
                        </thead>
                        <tbody className="bg-white divide-y divide-gray-200">
                            {filteredJournals.map((journal) => (
                                ...
                                >
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        <div>
                                            <div className="text-sm text-green-600">{isRTL ? arabicText.debit : 'D'}: {formatCurrency(journal.totalDebit, journal.currency)}</div>
                                            <div className="text-sm text-red-600">{isRTL ? arabicText.credit : 'C'}: {formatCurrency(journal.totalCredit, journal.currency)}</div>
                                        </div>
                                    </td>
                                    <td className="px-6 py-4 whitespace-nowrap">
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        ...
                                        {expandedJournals[journal._id] && (
                                            <div className={`absolute z-10 mt-2 p-2 bg-white shadow-lg rounded-lg border w-64 ${isRTL ? 'right-0' : 'left-0'}`}>
                                                {journal.entries?.map((entry, idx) => (
                                                    <div key={idx} className="text-xs p-1 border-b last:border-b-0">
                                                        ...
                                                        <div className="flex justify-between text-gray-600">
                                                            <span>{isRTL ? arabicText.debit : 'D'}: {formatCurrency(entry.debit, journal.currency)}</span>
                                                            <span>{isRTL ? arabicText.credit : 'C'}: {formatCurrency(entry.credit, journal.currency)}</span>
                                                        </div>
                                                    </div>
                                                ))}
                                            </div>
