...
// Currency options with Arabic names
const currencies = [
    ...
    const fetchContact = async () => {
        ...
        try {
            ...
            if (response.data.success) {
                ...
                toast.error('فشل جلب تفاصيل جهة الاتصال');
                navigate('/ar/contacts');
            }
        } catch (error) {
            console.error('Error fetching contact:', error);
            toast.error('خطأ في تحميل تفاصيل جهة الاتصال');
            navigate('/ar/contacts');
        } finally {
            setLoading(false);
        }
    ...
    const handleUpdateBalance = async () => {
        ...
        try {
            ...
            if (response.data.success) {
                ...
                if (journalCreated) {
                    toast.success('تم ترحيل الرصيد الى قيود اليوميه');
                } else {
            ...
            toast.error(error.response?.data?.message || 'فشل تحديث الرصيد');
        } finally {
            setUpdating(false);
        }
    };
...
const handlePayment = async () => {
    ...
    if (!paymentForm.paymentAccount) {
        toast.error('الرجاء تحديد حساب الدفع');
        return;
    }
    ...
    try {
        ...
        if (contact.isReceivable) {
            ...
            toast.error('نوع جهة الاتصال غير محدد');
        ...
        try {
            ...
            if (transactionResponse.data.success) {
                ...
                toast.error('تم إنشاء الفاتورة ولكن فشل حفظ المعاملة');
            }
        } catch (transactionError) {
            console.error('Transaction error details:', transactionError.response?.data || transactionError);
            toast.error('تم إنشاء الفاتورة ولكن تعذر تسجيل المعاملة');
        }
        ...
        try {
            ...
            if (journalResponse.data.success) {
                ...
                console.error('Journal response error:', journalResponse.data);
                toast.error('فشل إنشاء قيد اليومية');
            }
        } catch (journalError) {
            console.error('Journal error details:', journalError.response?.data || journalError);
            toast.error('تعذر إنشاء قيد اليومية');
        }
        ...
        if (invoiceCreated && transactionCreated && journalCreated) {
            toast.success('تم إتمام الدفع بنجاح مع قيد اليومية!');
        } else if (invoiceCreated && transactionCreated) {
            toast.success('تم تسجيل الدفع (الفاتورة والمعاملة)');
        } else if (invoiceCreated) {
            toast.success('تم تسجيل الدفع (الفاتورة فقط)');
        }
        ...
        if (!invoiceCreated) {
            toast.error('فشل إنشاء الفاتورة: ' + (error.response?.data?.message || error.message));
        } else {
            toast.error('خطأ في إكمال عملية الدفع');
        }
    ...
    // Handle edit
    const handleEdit = () => {
        navigate(`/ar/contacts/edit/${contact._id}`, { state: { contact } });
    };
    ...
    if (loading) {
        return (
            <div className="min-h-screen bg-gradient-to-br from-blue-50 to-sky-50 flex items-center justify-center" dir="rtl">
                <div className="text-center">
                    <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-600 mx-auto"></div>
                    <p className="mt-4 text-gray-600">جاري تحميل تفاصيل جهة الاتصال...</p>
                </div>
            </div>
        );
    }
        ...
        return (
            <div className="min-h-screen bg-gradient-to-br from-blue-50 to-sky-50 flex items-center justify-center" dir="rtl">
                <div className="text-center">
                    ...
                    <h3 className="text-xl font-semibold text-gray-700 mb-2">جهة الاتصال غير موجودة</h3>
                    <button
                        onClick={handleBack}
    ...
    return (
        <div className="w-full min-h-screen bg-gradient-to-br from-blue-50 to-sky-50" dir="rtl">
            {/* Header */}
                    ...
                    {activeTab === 'info' && (
                        <div className="space-y-6">
                            ...
                            <div className="border-t border-gray-100 pt-6">
                                <h2 className="text-lg font-semibold text-gray-800 mb-4">ملخص مالي</h2>
                        ...
                        <div>
                            ...
                            {transactionsLoading ? (
                                ...
                                <>
                                    ...
                                    <div className="print-statement hidden print:block mb-8">
                                        <div className="text-center mb-6">
                                            ...
                                            <p className="text-gray-600">جهة الاتصال: {contact.name}</p>
                                            <p className="text-gray-600">التاريخ: {new Date().toLocaleDateString('ar-AE')}</p>
                                            <p className="text-gray-600">الرصيد الحالي: {formatCurrency(contact.balance)}</p>
                                        </div>
                                    </div>
                                        ...
                                        <table className="w-full border-collapse">
                                            <thead>
                                                <tr className="bg-gray-100">
                                                    ...
                                                    <th className="border p-2 text-right">نوع الفاتورة</th>
                                                    <th className="border p-2 text-right">رقم الفاتورة</th>
                                                    <th className="border p-2 text-left">الإجمالي</th>
                                                    <th className="border p-2 text-left">الضريبة</th>
                                                    <th className="border p-2 text-left">الإجمالي مع الضريبة</th>
                                                    <th className="border p-2 text-left">المدفوع</th>
                                                    <th className="border p-2 text-left">الرصيد</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                {transactions.map((invoice, index) => {
                                                    ...
                                                    return (
                                                        <tr key={index} className="hover:bg-gray-50">
                                                            <td className="border p-2 text-right">{formatDate(invoice.invoiceDate)}</td>
                                                            <td className="border p-2 text-right">{invoice.invoiceType || 'N/A'}</td>
                                                            <td className="border p-2 text-right">{invoice.invoiceNumber || 'N/A'}</td>
                                                            <td className="border p-2 text-left">{formatNumber(invoice.bills?.total)}</td>
                                                            <td className="border p-2 text-left">{formatNumber(invoice.bills?.tax)}</td>
                                                            ...
                                                            <td className={`border p-2 text-left font-semibold ${runningBalance > 0 ? 'text-green-600' :
                                            ...
                                            <tfoot>
                                                <tr className="bg-gray-50 font-bold">
                                                    ...
                                                    <td className="border p-2 text-left">{formatNumber(totals.total)}</td>
                                                    <td className="border p-2 text-left">{formatNumber(totals.tax)}</td>
                                                    <td className="border p-2 text-left">{formatNumber(totals.totalWithTax)}</td>
                                                    <td className="border p-2 text-left">{formatNumber(totals.paid)}</td>
                                                    <td className="border p-2 text-left">{formatCurrency(contact.balance)}</td>
                                                </tr>
                                            </tfoot>
                                        </table>
                                    </div>
                                    ...
                                    <div className="mt-6 p-4 bg-gray-50 rounded-lg print:bg-gray-100">
                                        <h3 className="font-semibold text-gray-700 mb-2">ملخص</h3>
                                ...
                                <div className="text-center py-12 bg-gray-50 rounded-lg">
                                    ...
                                    <p className="text-gray-500">لا توجد معاملات لهذه جهة الاتصال</p>
                                </div>
                            )}
                        </div>
                    )}
                            ...
                            <div className="max-w-2xl mx-auto">
                                ...
                                <div className="bg-blue-50 rounded-lg p-4 mb-6 border border-blue-200">
                                    <p className="text-sm text-blue-800">
                                        <span className="font-semibold">تسجيل دفعة لـ:</span>{' '}
                                        {contact.isReceivable && contact.isPayable ? (
                                            <span>عميل/مورد - يرجى التأكد من اختيار الحساب الصحيح</span>
                                        ) : contact.isReceivable ? (
                                            <span>عميل - سيتم تسجيل الدفعة كدفعة عميل</span>
                                        ) : contact.isPayable ? (
                                            <span>مورد - سيتم تسجيل الدفعة كدفعة مورد</span>
                                        ) : (
                                            <span>نوع جهة الاتصال غير محدد</span>
                                        )}
                                    </p>
                                </div>
                                ...
                                <div className="bg-gray-50 rounded-xl p-6 border-2 border-gray-200">
                                    <div className="space-y-4">
                                        ...
                                        <div className="space-y-3">
                                            ...
                                            <div className="relative">
                                                ...
                                                <input
                                                    ...
                                                    placeholder="البحث عن حسابات..."
                                                    className="w-full px-4 py-3 border-2 border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent pr-10"
                                                />
                                                <FaSearch className="absolute right-3 top-10 transform -translate-y-1/2 text-gray-400" />
                                            </div>

                                            {/* Payment Account Select */}
                                            <div>
                                                <label className="block text-sm font-medium text-gray-700 mb-2">
                                                    حساب الدفع <span className="text-red-500">*</span>
                                                </label>
                                                {loadingAccounts ? (
                                                    <div className="text-center py-3 border-2 border-gray-200 rounded-lg">
                                                        <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-blue-600 mx-auto"></div>
                                                        <p className="text-xs text-gray-500 mt-1">جاري تحميل الحسابات...</p>
                                                    </div>
                                                ) : (
                                                    <select
                                        ...
                                        <div className="bg-blue-50 rounded-lg p-4 mt-4">
                                            <h3 className="font-semibold text-blue-800 mb-2">ملخص الدفع</h3>
                                            ...
                                            {selectedPaymentAccount && (
                                                <div className="mt-2 text-sm text-gray-600">
                                                    حساب الدفع: {selectedPaymentAccount.code} - {selectedPaymentAccount.nameArb || selectedPaymentAccount.name}
                                                </div>
                                            )}
                                        </div>
                        ...
                        <div>
                            ...
                            <div className="bg-gradient-to-br from-blue-50 to-sky-50 rounded-xl p-6 border-2 border-dashed border-blue-200">
                                ...
                                <div className="text-center mb-8">
                                    ...
                                    <h3 className="text-xl font-medium text-gray-800 mb-2">الرصيد الحالي</h3>
                                ...
                                {!showBalanceInput ? (
                                    <div className="text-center">
                                        ...
                                        <p className="text-sm text-gray-500 mt-3">
                                            تعيين أو تعديل الرصيد الافتتاحي لجهة الاتصال هذه
                                        </p>
                                    </div>
                                ) : (
...
//                 }
//             } else {
//                 toast.error('فشل في جلب تفاصيل جهة الاتصال');
...
//             // Step 4: Create Journal Entry with proper header information
//             try {
//                 // Determine journal header information based on selected payment account
//                 let journalName = '';
//                 let journalNameArb = '';
...
//                 // Helper function to get account-specific Arabic values
//                 const getAccountArabicValues = (account) => {
//                     // Check if this is Dubai Islamic Bank
//                     if (account.name === 'Dubai Islamic Bank' ||
//                         account.nameArb === 'بنك دبي الاسلامي' ||
//                         account.code === '111101') {
//                         return {
//                             accGroupArb: 'البنوك',
//                             accClassArb: 'النقديه ومافي حكمها',
//                             accLevelArb: 'الاصول المتداوله',
//                             accChartArb: 'الاصول'
//                         };
//                     }
//                     // Check if this is Cash
//                     else if (account.name === 'Cash' ||
//                         account.nameArb === 'الخزينة الرئيسيه' ||
//                         account.code === '111201') {
//                         return {
//                             accGroupArb: 'الصندوق',
//                             accClassArb: 'النقديه ومافي حكمها',
...
//                         };
//                     }
//                     // Default fallback - use account's own values or empty strings
//                     else {
//                         return {
//                             accGroupArb: account.groupArb || account.groupNameArb || '',
//                             accClassArb: account.classArb || account.classNameArb || '',
//                             accLevelArb: account.levelArb || account.levelNameArb || '',
//                             accChartArb: account.chartArb || account.chartNameArb || ''
//                         };
//                     }
//                 };

//                 // Helper function to create entry with all required fields
//                 const createEntry = (account, isDebit, amount, partnerInfo, description) => {
//                     const arabicValues = getAccountArabicValues(account);
...
//                         accName: account.name || '',
//                         accGroup: account.group || account.groupName || '',
//                         accClass: account.class || account.className || '',
//                         accLevel: account.level || account.levelName || '',
//                         accChart: account.chart || account.chartName || '',
//                         accType: account.type || 'Balance Sheet',

//                         // Account information (Arabic) - Using the specific values
//                         accNameArb: account.nameArb || account.name || '',
//                         accGroupArb: arabicValues.accGroupArb,
//                         accClassArb: arabicValues.accClassArb,
//                         accLevelArb: arabicValues.accLevelArb,
//                         accChartArb: arabicValues.accChartArb,
...
//                     // Debit entry for payment account (Dubai Islamic Bank or Cash)
//                     journalEntries.push(createEntry(
//                         debitAccount,
//                         true, // isDebit
//                         paymentAmount,
//                         { name: contact.name, type: 'customer', id: contact._id },
//                         `Payment received from ${contact.name}`
//                     ));

//                     // Credit entry for receivable account
//                     journalEntries.push(createEntry(
//                         creditAccount,
//                         false, // isDebit (credit)
//                         paymentAmount,
...
//                     // Debit entry for payable account
//                     journalEntries.push(createEntry(
//                         debitAccount,
//                         true, // isDebit
//                         paymentAmount,
//                         { name: contact.name, type: 'supplier', id: contact._id },
//                         `Payment made to ${contact.name}`
//                     ));

//                     // Credit entry for payment account (Dubai Islamic Bank or Cash)
//                     journalEntries.push(createEntry(
//                         creditAccount,
//                         false, // isDebit (credit)
//                         paymentAmount,
...
//                 const year = paymentDate.getFullYear();
//                 const month = String(paymentDate.getMonth() + 1).padStart(2, '0');
//                 const period = `${year}-${month}`; // Format: YYYY-MM
...
//                 console.log('Journal Data being sent:', JSON.stringify(journalData, null, 2));
//                 console.log('First entry sample:', journalData.entries[0]);
...
//     // Get contact type badge in Arabic
//     const getContactTypeBadge = () => {
//         if (!contact) return null;
...
//                                             </div>
//                                             <div>
//                                                 <p className="text-xs text-gray-500">فئة الحساب</p>
//                                                 <p className="font-medium text-gray-800">{contact.accReceivableClass}</p>
//                                             </div>
//                                             <div>
//                                                 <p className="text-xs text-gray-500">مستوى الحساب</p>
...
//                                             </div>
//                                             <div>
//                                                 <p className="text-xs text-gray-500">دليل الحسابات</p>
//                                                 <p className="font-medium text-gray-800">{contact.accReceivableChart}</p>
//                                             </div>
//                                             <div>
//                                                 <p className="text-xs text-gray-500">نوع الحساب</p>
...
//                                     <div className="print-statement hidden print:block mb-8">
//                                         <div className="text-center mb-6">
//                                             <h1 className="text-2xl font-bold text-gray-800">كشف حساب المعاملات</h1>
//                                             <p className="text-gray-600">جهة الاتصال: {contact.name}</p>
//                                             <p className="text-gray-600">التاريخ: {new Date().toLocaleDateString('ar-SA')}</p>
//                                             <p className="text-gray-600">الرصيد الحالي: {formatCurrency(contact.balance)}</p>
//                                         </div>
//                                     </div>
...
//                                                     <th className="border p-2 text-left">الإجمالي</th>
//                                                     <th className="border p-2 text-left">الضريبة</th>
//                                                     <th className="border p-2 text-left">الإجمالي شامل الضريبة</th>
//                                                     <th className="border p-2 text-left">المدفوع</th>
//                                                     <th className="border p-2 text-left">الرصيد</th>
//                                                 </tr>
//                                             </thead>
...
//                                     {/* Summary Section */}
//                                     <div className="mt-6 p-4 bg-gray-50 rounded-lg print:bg-gray-100">
//                                         <h3 className="font-semibold text-gray-700 mb-2">الملخص</h3>
//                                         <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
//                                             <div>
//                                                 <p className="text-sm text-gray-600">إجمالي الفواتير</p>
//                                                 <p className="text-lg font-bold">{transactions.length}</p>
...
//                                 <div className="text-center py-12 bg-gray-50 rounded-lg">
//                                     <FaHistory className="mx-auto text-4xl text-gray-400 mb-3" />
//                                     <p className="text-gray-500">لا توجد معاملات لجهة الاتصال هذه</p>
//                                 </div>
//                             )}
//                         </div>
//                     )}
