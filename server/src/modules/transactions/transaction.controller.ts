...
import { AddTransactionService, updateTransactionService, getTransactionService } from './transaction.service';

// Add Transaction Controller
export const addTransactionController = async (req: Request, res: Response): Promise<void> => {
    try {
        // 1. Basic validation
        ...
        // 4. Call service
        const result = await AddTransactionService(transactionData);
        ...
        const message = 
            error.message.includes('already exists') ? 'Transaction already exists' :
...
export const getTransactionsController = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const result = await getTransactionService(page, limit, search, filters);
...
export const getTransactionStatsController = async (req: Request, res: Response): Promise<void> => {
    try {
        const Transaction = require('./transaction.model').default;
